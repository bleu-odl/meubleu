...
import { createClient } from '../../lib/supabase' // Cliente do navegador para as ações (DELETE/UPDATE)
...
export default function IncomesClient({ initialIncomes, kpiData, selectedMonth, selectedYear }: IncomesClientProps) {
  ...
  // Filtragem local (apenas busca por texto, já que o mês/ano vem filtrado do servidor)
  const filteredIncomes = initialIncomes.filter(inc => 
    inc.description.toLowerCase().includes(searchTerm.toLowerCase())
  )

  // Atualizar URL ao mudar filtros (Server Fetching)
                    ...
                    <table className="min-w-full divide-y divide-white/5">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {filteredIncomes.length === 0 ? (
                                <tr><td colSpan={4} className="p-12 text-center text-zinc-500 text-xs">Nenhuma receita encontrada.</td></tr>
                            ) : (
