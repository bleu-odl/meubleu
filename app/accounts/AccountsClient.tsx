...
export default function AccountsClient({ initialAccounts }: AccountsClientProps) {
  ...
  async function handleDelete(id: string) { 
    if (!confirm('Tem certeza? Isso pode afetar despesas vinculadas.')) return
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
        <div className="mb-8">
            ...
            <p className="text-zinc-400 mt-1 text-sm">Organize suas categorias de gastos.</p>
        </div>
        ...
        <div className="card rounded-xl p-0 overflow-hidden bg-zinc-900/50">
          {filteredAccounts.length === 0 ? (
              <div className="p-12 text-center text-zinc-500 flex flex-col items-center text-sm"><Tag size={40} className="text-zinc-700 mb-3"/><p>Nada encontrado.</p></div>
          ) : (
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                  <SortableContext items={filteredAccounts.map(a => a.id)} strategy={verticalListSortingStrategy}>
                      <ul className="divide-y divide-white/5">
                          {filteredAccounts.map((acc, index) => (
                              <SortableAccountItem key={acc.id} account={acc} index={index} total={filteredAccounts.length} openEditModal={openEditModal} handleDelete={handleDelete} openMenuId={openMenuId} setOpenMenuId={setOpenMenuId} menuRef={menuRef}/>
                          ))}
                      </ul>
                  </SortableContext>
              </DndContext>
