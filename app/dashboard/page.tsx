...
// --- CORES ATUALIZADAS (MAIS SUAVES/PREMIUM) ---
const colors = { 
  // Substituímos o Vermelho puro por Rose (mais moderno/suave)
  red: '#F43F5E',    
  
  // Substituímos o Verde por Indigo (Cor da marca = Receita) ou Emerald mais suave
  green: '#6366F1',  // Indigo (Marca)
  // Se preferir manter verde para receitas, use: '#34D399' (Emerald suave)

  yellow: '#FBBF24', // Amber mais suave
  blue: '#818CF8',   
  slate: '#0F172A',
  indigo: '#6366F1'
}
...
export default function DashboardPage() {
  ...
  async function fetchDashboardData() {
    ...
    const startCurrent = new Date(selectedYear, selectedMonth, 1).toISOString()
    const endCurrent = new Date(selectedYear, selectedMonth + 1, 0).toISOString()
    const startLast = new Date(selectedYear, selectedMonth - 1, 1).toISOString()
    const endLast = new Date(selectedYear, selectedMonth, 0).toISOString()
    ...
    yearExpenses?.forEach(exp => {
        const d = new Date(exp.date); const key = `${d.getFullYear()}-${d.getMonth()}`;
        if (monthlyDataMap.has(key)) { const c = monthlyDataMap.get(key); c.expense += exp.value; monthlyDataMap.set(key, c) }
    })
    yearIncomes?.forEach(inc => {
        const d = new Date(inc.date); const key = `${d.getFullYear()}-${d.getMonth()}`;
        if (monthlyDataMap.has(key)) { const c = monthlyDataMap.get(key); c.income += inc.amount; monthlyDataMap.set(key, c) }
    })
  ...
  function processSpecificChart(accountName: string, allExpenses: any[]) {
    ...
    const tempMap = new Map()
     ...
     allExpenses.filter(e => e.name === accountName).forEach(exp => {
        const d = new Date(exp.date); const key = `${d.getFullYear()}-${d.getMonth()}`;
        if (tempMap.has(key)) { const c = tempMap.get(key); c.value += exp.value; tempMap.set(key, c) }
     })
  ...
  // Configuração do Texto de Score (Cores atualizadas)
  let scoreTextColor = 'text-red-400' // Rose suave para critico
  let scoreLabel = 'Crítico'
  let scoreDesc = 'Seus gastos estão excedendo sua renda. Atenção máxima.'
  
  if (healthScore >= 80) {
    scoreTextColor = 'text-indigo-400' // Indigo para excelente (match com Receita)
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          <div className={cardClass}>
            <div>
              ...
              <h3 className="text-[30px] font-bold text-white">R$ {currentMonthTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
            </div>
          ...
          <div className={cardClass}>
            <div className="w-full"><p className="text-[13px] font-medium text-slate-400 mb-1">Próximo vencimento</p>{nextDue ? (<><h3 className="text-lg font-bold text-white truncate" title={nextDue.name}>{nextDue.name}</h3><p className="text-[22px] font-bold text-indigo-400 mt-0.5">R$ {nextDue.value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p><p className="text-[11px] font-medium text-slate-500 mt-1">Vence {new Date(nextDue.date).toLocaleDateString('pt-BR', {timeZone: 'UTC'})}</p></>) : (<div className="mt-2"><h3 className="text-sm font-bold text-emerald-400">Nenhum pendente 🎉</h3></div>)}</div>
            <div className={iconBadgeClass}><CalendarClock size={20} strokeWidth={2.5} /></div>
          </div>
        </div>

        {/* GRÁFICO 1 (Cores Atualizadas: Indigo vs Rose) */}
            ...
            <div className="mb-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div className="bg-[#1E1F2B] p-1 rounded-xl border border-white/5 flex">
                    {[
                        ...
                        const isActive = chartFilter === filter.key
                        return (
                            <button
                                key={filter.key}
                    ...
                    <LineChart data={chartData} margin={{ top: 5, right: 10, bottom: 5, left: -10 }}>
                        ...
                        {(chartFilter === 'all' || chartFilter === 'expense') && (
                            <Line 
                                ...
                                stroke={colors.red} 
                                strokeWidth={3} 
                                dot={{ r: 4, fill: colors.red, strokeWidth: 2, stroke: "#1E1F2B" }} 
                        ...
                        {(chartFilter === 'all' || chartFilter === 'income') && (
                            <Line 
                                ...
                                stroke={colors.green} 
                                strokeWidth={3} 
                                dot={{ r: 4, fill: colors.green, strokeWidth: 2, stroke: "#1E1F2B" }} 
                  ...
                  {ccCategoryData.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={ccCategoryData} layout="vertical" margin={{ top: 5, right: 30, left: 40, bottom: 5 }}>
                        ...
                        <Tooltip cursor={{ fill: '#ffffff08' }} contentStyle={{ borderRadius: '8px', border: 'none', background: '#1E1F2B', boxShadow: '0 10px 30px -5px rgba(0,0,0,0.3)' }} itemStyle={{color: '#fff'}} formatter={(value: number) => [`R$ ${value.toLocaleString('pt-BR')}`, 'Gasto']} />
                    ...
                    return (
                      <div key={cat.name} className="border border-white/5 rounded-xl overflow-hidden bg-[#23242f] shrink-0">
                        <button onClick={() => setExpandedCategory(isOpen ? null : cat.name)} className="w-full flex items-center justify-between p-4 hover:bg-white/5 transition-colors">
                          ...
                          <div className="flex items-center gap-3"><span className="text-sm text-slate-300 font-medium">R$ {cat.value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>{isOpen ? <ChevronUp size={16} className="text-slate-500"/> : <ChevronDown size={16} className="text-slate-500"/>}</div>
                        </button>
                        {isOpen && (
                            ...
                            <ul className="divide-y divide-white/5">
                              {getTransactionsByCategory(cat.name).map((t, idx) => (
                                <li key={idx} className="py-2.5 flex justify-between items-center text-xs">
                                  ...
                                  <div className="text-white font-medium whitespace-nowrap">R$ {t.amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</div>
                                </li>
                              ))}
